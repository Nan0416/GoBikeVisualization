 ...
 *  */
function add_to(station_name, is_pick, month, hour){
    let condition = (
    ...
    if(!condition){
        console.log(station_name, is_pick, month, hour);
        return;
    }
    ...
    if(is_pick){
        data_container.get(station_name)[month].sum[hour]++;
        data_container.get(station_name)[month].diff[hour]--;
        data_container.get(station_name)[month].pick[hour]++;
    }else{
        data_container.get(station_name)[month].sum[hour]++;
        data_container.get(station_name)[month].diff[hour]++;
        data_container.get(station_name)[month].return[hour]++;
    }
}
    ...
    async.each(raw_filenames, (filename, callback)=>{
        ...
        .on('data', function(data){
            try{
                ...
                add_to(start_station, true, startTime.getMonth(), startTime.getHours());
                add_to(end_station, false, endTime.getMonth(), endTime.getHours());
            }catch(err){
                console.log(err)
            }
        })
